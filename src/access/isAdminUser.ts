...
/** Admin or moderator — create/start/end livestreams and open broadcaster UI. */
export function canBroadcastLivestream(user: PayloadRequest['user']): user is User {
  if (!user || typeof user !== 'object') return false
  if (!('collection' in user)) return false
  if ((user as { collection: string }).collection !== 'users') return false
  const u = user as User
  return Boolean(u.roles?.includes('admin') || u.roles?.includes('moderator'))
}
