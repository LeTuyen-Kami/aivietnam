...
- Migration SQL must be idempotent:
  ...
  - Wrap FK / enum creation in `DO $$ BEGIN ... EXCEPTION WHEN duplicate_object THEN NULL; END $$;`
- Keep each migration narrowly scoped to the intended change; avoid unrelated drops/renames from schema drift.
- Always register new migration files in `src/migrations/index.ts`.
- If Payload prompts that dev mode previously pushed schema, run migrate non-interactively with explicit confirmation in CI-safe flows.
